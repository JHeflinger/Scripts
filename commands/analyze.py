...
description: given a directory to analyze, performs basic analytics on codeases
"""
...
def analyze_code(directories, typedes):
    total_sloc = 0
    total_files = 0
    total_hdrs = 0
    total_srcs = 0
    ...
    for directory in directories:
        for root, dirs, files in os.walk(directory):
            for file in files:
                ...
                current_sloc = 0
                good_file = False
                if (len(filepath) > 4 and ".cpp" == filepath[-4:]) or (len(filepath) > 2 and ".c" == filepath[-2:]):
                    total_srcs += 1
                    good_file = True
                if (len(filepath) > 2 and ".h" == filepath[-2:]):
                    total_hdrs += 1
                    good_file = True
                if (len(filepath) > 5 and ".comp" == filepath[-5:]):
                    total_cshaders += 1
                    good_file = True
                total_files += 1
                if good_file:
    ...
    return (typedes + " ANALYSIS: \n" + 
            ...
            "├──────────────────┼───────────────┤\n"
            "│9. COMP SHADERS   │" + pad_num(total_cshaders) + "│\n" +
            "└──────────────────┴───────────────┘\n")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        srcdir = sys.argv[1]
    statement += analyze_code([srcdir, shaderdir], "CODEBASE") + "\n"
